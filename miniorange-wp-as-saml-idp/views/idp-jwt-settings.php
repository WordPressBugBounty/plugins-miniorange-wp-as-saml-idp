<?php
/**
 * This is the file for displaying the view when
 * a JWT Service Provider is being configured
...
if ( ! empty( $sp ) || $sp_exists ) {
	show_protocol_options( $sp, $protocol_inuse );
	echo '
                <div class="mo-idp-sp-data-manual mo-idp-bg" >';
	echo ' <a href="' . esc_url( $license_url ) . '" target="_blank" class="mo-idp-upload-data-anchor">';
			echo ' <div id="mo-idp-advt-premium-feature">';
				echo '  <img class="mo-idp-advt-premium-feature-lock" src="' . esc_url( MSI_URL ) . 'includes/images/lock.png"/>';
	...
	echo '<input type="hidden" name="service_provider" value="' . ( isset( $sp ) && ! empty( $sp ) ? esc_attr( $sp->id ) : '' ) . '" />
                            ...
                            <form id="add_sp" method="get" action="' . esc_url( $post_url ) . '">
                                <input type="hidden" name="page" value="' . esc_attr( $sp_settings_tab_details->menu_slug ) . '"/>
